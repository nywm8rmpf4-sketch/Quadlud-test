...
function startingSeed(engine,diff,plan){const d=clone(plan?.startingDeduction||plan?.deduction);if(!d)throw new Error('Tutor plan missing starting deduction');const sig=signature(d),matches=directMatches(engine,diff,sig);if(!plan.advancedStart&&sig&&matches.length===1)return [0,sig];return [1,d]}
...
const entries=[],seen=new Map(),plannerTimes=[];let directStarts=0,materializedStarts=0,directDisplays=0,materializedDisplays=0,totalSteps=0;
for(let poolIndex=0;poolIndex<sourceEntries.length;poolIndex++){
  ...
  for(let moveIndex=0;moveIndex<72;moveIndex++){
    ...
    start[0]===0?directStarts++:materializedStarts++;display[0]===0?directDisplays++:materializedDisplays++;
    const record=[fingerprint,start[0],cellIndex(plan.target),plan.value,clone(start[1]),selectionMeta(plan),display[0],clone(display[1]),proofMeta(plan)];
...
const payload={schema:1,version:'tango-tutor-sync-shard-r1',difficulty,poolVersion:Pool.version,tutorContract:{schema:tutorContract.schema,version:tutorContract.version,algorithm:tutorContract.algorithm,digest:tutorContract.digest},tutorPlannerToken:Tutor.TOKEN||null,humanPolicy:Human.POLICY||null,proofPolicy:Runtime.HUMAN_PROOF_POLICY||null,entryShape:'[poolIndex,initialFingerprint,steps]',stepShape:'[fingerprint,startKind,targetIndex,value,startPayload,selectionMeta,displayKind,displayPayload,proofMeta]',entries};
fs.mkdirSync(path.dirname(outputPath),{recursive:true});fs.writeFileSync(outputPath,JSON.stringify(payload));
const report={schema:1,version:payload.version,difficulty,poolVersion:payload.poolVersion,tutorContract:payload.tutorContract,tutorPlannerToken:payload.tutorPlannerToken,humanPolicy:payload.humanPolicy,proofPolicy:payload.proofPolicy,puzzles:entries.length,totalSteps,uniqueFingerprints:seen.size,directStarts,materializedStarts,directDisplays,materializedDisplays,bytes:fs.statSync(outputPath).size,livePlannerMs:stats(plannerTimes)};
