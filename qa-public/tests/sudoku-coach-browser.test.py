...
def text(page):return page.locator('#hintNotice .hint-notice-text').inner_text()
def more(page):
    b=page.locator('#hintNotice .coach-window-more');assert b.count()==1 and b.is_visible();b.click();return text(page)
def apply(page):
    b=page.locator('#coachWindowApply');assert b.count()==1 and b.is_visible();b.click()
with sync_playwright() as p:
    ...
    page.click('#hintBtn');t1=text(page);assert 'A6' in t1 and filled(page)==n0,t1;persona=coach_persona(page);assert persona and persona['count']==1 and persona['persona']=='guide' and persona['hidden']=='true',persona
    t2=more(page);assert 'Candidat unique' in t2 and '6' in t2 and filled(page)==n0,t2;assert coach_persona(page)['count']==1
    t3=more(page);assert '6' in t3 and filled(page)==n0,t3;assert page.locator('#coachWindowApply').count()==1
    apply(page);assert filled(page)==n0+1;assert page.evaluate('current.state[0][5]')==6 and page.evaluate('current.sol[0][5]')==1
