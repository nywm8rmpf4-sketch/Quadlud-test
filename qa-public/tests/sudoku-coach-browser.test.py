...
with sync_playwright() as p:
    ...
    assert page.evaluate("typeof QuadludCoachPresentationRuntime==='object' && QuadludCoachPresentationRuntime.VERSION===1");assert page.evaluate("coachStageBlock.__quadludD2Bridge===true");assert page.evaluate("hintStage.__quadludD4PersonaBridge===true")
    ...
    page.click('#hintBtn');t1=page.locator('#hintNotice .hint-notice-text').inner_text();assert '1/3' in t1 and 'A6' in t1 and filled(page)==n0,t1
    page.click('#hintBtn');t2=page.locator('#hintNotice .hint-notice-text').inner_text();assert '2/3' in t2 and 'Candidat unique' in t2 and '6' in t2 and filled(page)==n0,t2
    page.click('#hintBtn');t3=page.locator('#hintNotice .hint-notice-text').inner_text();assert '3/3' in t3 and '6' in t3 and filled(page)==n0+1,t3;assert page.evaluate('current.state[0][5]')==6 and page.evaluate('current.sol[0][5]')==1
