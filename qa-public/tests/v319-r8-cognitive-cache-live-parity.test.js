...
assert.strictEqual(info.dataSchema,9);assert.strictEqual(info.dataVersion,'tango-tutor-cache-r8-sync4-cognitive-dag');assert.strictEqual(info.contract?.version,2);assert.strictEqual(data.schema,9);assert(Cache._test.validMaterializedDag(data.materializedDag));assert.strictEqual(data.cognitiveModel,'quadlud-cognitive-load-v1');assert.strictEqual(data.cognitivePatternCatalog,'tango-cognitive-patterns-v1');
...
const report={schema:3,checkedStates:0,hits:0,lookupMisses:0,validationMisses:0,mismatches:0,maxCacheMs:0,maxLiveMs:0,byDifficulty:{},examples:[]};
for(const diff of DIFFS){const entries=pool.pools?.[diff];assert(Array.isArray(entries)&&entries.length===120);const stats={puzzles:count,states:0,hits:0,lookupMisses:0,validationMisses:0,mismatches:0,maxCacheMs:0,maxLiveMs:0};
  for(let poolIndex=start;poolIndex<end;poolIndex++){const entry=entries[poolIndex],state=stateFor(entry);
    for(let moveIndex=0;moveIndex<72;moveIndex++){if(!state.some(r=>r.includes(-1)))break;const engine=Planner.sessionFromPublicBoard(puzzle(entry,state),state),lookup=Cache.lookup(engine,diff);Cache._test.resetStats();
      ...
      if(reason){stats.mismatches++;report.mismatches++;if(report.examples.length<50)report.examples.push({diff,poolIndex,moveIndex,reason,cacheMs:Number(cacheMs.toFixed(3)),liveMs:Number(liveMs.toFixed(3)),cachedProof:proofKey(cached),liveProof:proofKey(live)})}
