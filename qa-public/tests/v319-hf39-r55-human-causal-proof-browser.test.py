...
def install_exact_a3_fixture(page):
 ...
 const cp=v=>JSON.parse(JSON.stringify(v)), state=Array.from({length:6},()=>Array(6).fill(-1));state[2][0]=0;state[5][1]=0;state[5][2]=1;const initial={state:cp(state)};
 ...
 walkthroughSession.base.edges=[[1,2,'r','×'],[0,3,'d','='],[1,3,'r','×'],[4,4,'d','×']];walkthroughSession.base.n=6;walkthroughSession.base.game='tango';walkthroughSession.base.diff='expert';walkthroughSession.initial=cp(initial);walkthroughSession.moves=raw;walkthroughSession.pedagogyNavigationByMove=[];walkthroughSession.done=true;walkthroughSession.stalled=false;walkthroughSession.atStart=false;
...
def main():
 ...
 with sync_playwright() as p:
  ...
  human.load_runtime(page);human.install_human_fixture(page);page.locator('#walkthroughBtn').click();page.wait_for_selector('.walkthrough-panel');page.wait_for_timeout(100);fixture=install_exact_a3_fixture(page);assert fixture['count']==12,fixture;assert fixture['kinds']==['hypothesis']+['reasoning']*8+['contradiction','rollback','action'],fixture
  states=[]
  ...
  assert not errors,errors;assert states[0]['markers']==[{'label':'H','cell':'A3'}],states[0];expected=['B3','B4','A4','B5','E3','E5','F5','A5'];reasoning=[s for s in states if s['kind']=='reasoning'];assert [s['produced'] for s in reasoning]==expected,[(s['produced'],s['text']) for s in reasoning]
  ...
  assert 'B3' in states[2]['text'] and 'B4' in states[2]['text'];assert 'E3' in states[5]['text'];assert 'E3' in states[6]['text'] and 'E5' in states[6]['text'];assert states[9]['kind']=='contradiction' and 'colonne 5' in states[9]['text'].lower();assert states[10]['kind']=='rollback' and 'contradiction vient d’être établie' in states[10]['text'];assert states[11]['kind']=='action' and states[11]['actionVisible'] and 'conclusion réelle' in states[11]['text'].lower() and 'A3' in states[11]['text'];context.close();browser.close()
 (EVIDENCE/'summary.json').write_text(json.dumps({'fixture':fixture,'states':states,'errors':errors},ensure_ascii=False,indent=2),encoding='utf-8');print('PASS HF3.9-R5.5 exact A3 browser proof: 12 atomic screens H,B3,B4,A4,B5,E3,E5,F5,A5,contradiction,rollback,action; mobile evidence captured.')
