...
def resolve_web_root() -> Path:
    ...
    candidates = [
        here.parents[1] / "GitHub",  # canonical private/public QA layout
        here.parents[2],             # direct execution from qa-public/tests
    ]
    for candidate in candidates:
        if (candidate / "index.html").is_file():
            return candidate
    raise FileNotFoundError("QUADLUD runtime root not found")
...
def main() -> None:
    ...
    (evidence_dir / "review.md").write_text(
        ...
        "Leur analyse séquentielle doit être effectuée avant toute déclaration « prête pour test humain ». "
