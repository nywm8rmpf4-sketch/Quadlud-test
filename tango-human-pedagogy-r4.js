...
function cognitiveProofStagesForDeduction(d,presenter){
  const raw=proofStagesForDeduction(d,presenter),bridge=root.QuadludTangoCognitiveProofStagesBridge;
  return typeof bridge?.compactProofStages==='function'?bridge.compactProofStages(d,raw):raw
}
...
function installCoach(){
  ...
  const coherent=function(_raw){let engine;try{engine=tangoLogicSession()}catch(error){return coachFailure({status:'error',error},null)}const presenter=tangoReasoningPresenter(),plan=chooseGloballySimplestPlan(engine,current?.diff);if(plan?.status!=='move')return coachFailure(plan,presenter);const activeDeduction=plan.displayDeduction||plan.deduction,stages=cognitiveProofStagesForDeduction(activeDeduction,presenter),boardKey=historySnapshotKey(),sig=plan.humanSignature||`${moveKey(plan)}|${plan.displayProof?.kind||''}`,flow=current.hintFlow,isSame=flow?.kind==='tango-human-progressive-proof'&&flow.boardKey===boardKey&&flow.signature===sig,total=stages.length+1;
    ...
    const active=flow.plan||plan,activeD=active.displayDeduction||active.deduction,activeStages=cognitiveProofStagesForDeduction(activeD,presenter),next=Math.min(activeStages.length,Number(flow.stageIndex||0)+1),stage=activeStages[next-1];if(!stage)return coachFailure({status:'error',reason:'missing-stage'},presenter);
...
function installEarly(){
  const upgraded=Object.freeze({...Base,VERSION:7,HUMAN_PROOF_POLICY:POLICY,planHumanMove:chooseGloballySimplestPlan,_test:Object.freeze({...Base._test,playableDirectPlans,evaluatePlanHumanProof,compareHumanCandidate,chooseGloballySimplestPlan,proofStagesForDeduction:cognitiveProofStagesForDeduction,rawProofStagesForDeduction:proofStagesForDeduction,stageFocusDeduction,traceForAdvanced,concreteExplanation})});root.QuadludTangoPlayedMoveRuntime=upgraded;root.walkthroughGenerateTangoNext=walkthroughGenerateHumanNext;
...
return Object.freeze({VERSION,POLICY,installEarly,installLate,chooseGloballySimplestPlan,proofStagesForDeduction:cognitiveProofStagesForDeduction,rawProofStagesForDeduction:proofStagesForDeduction,decorateProgressiveProof,_test:Object.freeze({playableDirectPlans,evaluatePlanHumanProof,compareHumanCandidate,chooseGloballySimplestPlan,proofStagesForDeduction:cognitiveProofStagesForDeduction,rawProofStagesForDeduction:proofStagesForDeduction,stageFocusDeduction,traceForAdvanced,concreteExplanation,moveKey,planStableKey})});
});