...
function concreteExplanation(p,d,h){
  ...
  if(d?.rule==='RELATION_BALANCE'&&relations.length){const rel=relations[0],a=cell(...rel.a),b=cell(...rel.b),parity=relationParity(rel),symbol=parity===0?'=':'×',outValues=(d?.conclusions||[]).filter(c=>c?.type==='VALUE');if(parity===0&&outValues.length>=2&&outValues.every(c=>Number(c.value)===Number(outValues[0].value))){const forced=Number(outValues[0].value),rejected=1-forced,knownRejected=values.filter(v=>Number(v.value)===rejected);if(knownRejected.length>=2){const known=knownRejected.map(v=>cell(...v.cell)).join(' et ');return fr?`${known} contiennent déjà ${piece(rejected)} dans ${unit}. La relation ${symbol} impose que ${a} et ${b} soient identiques. Si elles étaient aussi ${piece(rejected)}, il y en aurait trop pour l’équilibre. Donc ${a} et ${b} = ${piece(forced)}.`:`${known} already contain ${piece(rejected)} in ${unit}. The ${symbol} relation requires ${a} and ${b} to be identical. If they were also ${piece(rejected)}, the balance quota would be exceeded. Therefore ${a} and ${b} = ${piece(forced)}.`}}return fr?`Dans ${unit}, la relation ${symbol} relie ${a} et ${b}. Avec les symboles déjà présents, une seule orientation de cette relation respecte l’équilibre. Donc ${conclusion}.`:`In ${unit}, the ${symbol} relation links ${a} and ${b}. With the symbols already present, only one orientation of this relation preserves balance. Therefore ${conclusion}.`}
