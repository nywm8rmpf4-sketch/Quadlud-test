...
const VERSION=2;
const COST_MODEL='pedagogy-easiest-next-attention-v2';
...
function normalizeConclusion(value){if(!value||!isCell(value.cell)||(Number(value.value)!==0&&Number(value.value)!==1))return null;return {cell:[Number(value.cell[0]),Number(value.cell[1])],value:Number(value.value)}}
function conclusionKey(value){const c=normalizeConclusion(value);return c?`${cellKey(c.cell)}:${c.value}`:''}
function uniqConclusions(values){const seen=new Set(),out=[];for(const value of values||[]){const c=normalizeConclusion(value),key=conclusionKey(c);if(!c||seen.has(key))continue;seen.add(key);out.push(c)}return out}
...
function candidateConclusion(candidate){
  if(isCell(candidate?.target)&&(Number(candidate?.value)===0||Number(candidate?.value)===1))return {cell:[Number(candidate.target[0]),Number(candidate.target[1])],value:Number(candidate.value)};
  if(isCell(candidate?.target)&&(Number(candidate?.targetValue)===0||Number(candidate?.targetValue)===1))return {cell:[Number(candidate.target[0]),Number(candidate.target[1])],value:Number(candidate.targetValue)};
  const plan=candidate?.payload||candidate?.plan;
  if(isCell(plan?.target)&&(Number(plan?.value)===0||Number(plan?.value)===1))return {cell:[Number(plan.target[0]),Number(plan.target[1])],value:Number(plan.value)};
  return null
}
function contextualMetrics(candidate,context={}){
  ...
  const pendingConclusions=uniqConclusions(context.pendingConclusions||context.recentConclusions||[]),pendingKeys=new Set(pendingConclusions.map(conclusionKey)),candidateKey=conclusionKey(candidateConclusion(candidate));
  const pendingConclusionMatch=!!candidateKey&&pendingKeys.has(candidateKey);
  ...
  const pendingConclusionPenalty=pendingConclusions.length?(pendingConclusionMatch?0:1):0;
  const attentionDistance=minAttentionDistance(candidate.target,focusCells,recentCells);
  return Object.freeze({recentCellCount:recentCells.length,pendingConclusionCount:pendingConclusions.length,pendingConclusionMatch,pendingConclusionPenalty,premiseCount:premiseCells.length,reusedPremiseCount,novelPremiseCount,continuityPenalty,attentionDistance});
}
function contextualCost(candidate,context={}){
  ...
  return Object.freeze([...structural,m.pendingConclusionPenalty,m.continuityPenalty,m.novelPremiseCount,m.attentionDistance,...residual,base[3]||0]);
}
function rank(candidates,context={}){
  ...
  return Object.freeze({schema:2,costModel:COST_MODEL,selected:scored[0]||null,ranked:Object.freeze(scored)});
}

return Object.freeze({VERSION,COST_MODEL,rank,contextualCost,contextualMetrics,_test:Object.freeze({uniqCells,uniqConclusions,conclusionKey,candidateConclusion,minAttentionDistance,compareVectors})});
});
