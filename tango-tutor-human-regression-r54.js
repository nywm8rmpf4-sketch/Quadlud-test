...
function relationBalanceDetail(move,loc=locale()){
  ...
  if(loc==='fr'){const known=[];if(sunCells.length)known.push(`${sunCells.join(' et ')} ${sunCells.length===1?'est déjà un soleil':'sont déjà des soleils'}`);if(moonCells.length)known.push(`${moonCells.join(' et ')} ${moonCells.length===1?'est déjà une lune':'sont déjà des lunes'}`);return {where:`Indice ${a} ${symbol} ${b} ; équilibre de ${unitText}.`,steps:[`${a} ${symbol} ${b} impose des symboles opposés : 1 soleil + 1 lune.`,`${known.join(' ; ')}. Avec ${a}/${b} : ${suns} soleil${suns>1?'s':''}, ${moons} lune${moons>1?'s':''} ; ${t} reste à fixer.`,`Pour atteindre ${quota} soleils et ${quota} lunes, ${t} = ${targetValue===1?'soleil ☀':'lune ☾'}.`]}}
  const known=[];if(sunCells.length)known.push(`${sunCells.join(' and ')} ${sunCells.length===1?'is already a sun':'are already suns'}`);if(moonCells.length)known.push(`${moonCells.join(' and ')} ${moonCells.length===1?'is already a moon':'are already moons'}`);return {where:`${a} ${symbol} ${b} clue; balance of ${unitText}.`,steps:[`${a} ${symbol} ${b} requires opposite symbols: 1 sun + 1 moon.`,`${known.join('; ')}. With ${a}/${b}: ${suns} sun${suns===1?'':'s'}, ${moons} moon${moons===1?'':'s'}; ${t} remains.`,`To reach ${quota} suns and ${quota} moons, ${t} = ${targetValue===1?'sun ☀':'moon ☾'}.`]}
}
