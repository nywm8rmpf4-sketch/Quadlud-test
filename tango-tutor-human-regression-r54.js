...
function relationBalanceDetail(move,loc=locale()){
  ...
  if(loc==='fr'){const known=[];if(sunCells.length)known.push(`${sunCells.join(' et ')} ${sunCells.length===1?'est soleil':'sont soleils'}`);if(moonCells.length)known.push(`${moonCells.join(' et ')} ${moonCells.length===1?'est lune':'sont lunes'}`);return {where:`Regarde ${a} ${symbol} ${b} et ${unitText}.`,steps:[`${a} ${symbol} ${b} : symboles opposés, donc 1 soleil et 1 lune.`,`${known.join(' ; ')}. Avec ce couple, ${unitText} compte ${suns} soleil${suns>1?'s':''} et ${moons} lune${moons>1?'s':''}. Pour arriver à ${quota} de chaque, ${t} = ${targetValue===1?'soleil ☀':'lune ☾'}.`]}}
  const known=[];if(sunCells.length)known.push(`${sunCells.join(' and ')} ${sunCells.length===1?'is a sun':'are suns'}`);if(moonCells.length)known.push(`${moonCells.join(' and ')} ${moonCells.length===1?'is a moon':'are moons'}`);return {where:`Look at ${a} ${symbol} ${b} and ${unitText}.`,steps:[`${a} ${symbol} ${b}: opposite symbols, so 1 sun and 1 moon.`,`${known.join('; ')}. With this pair, ${unitText} has ${suns} sun${suns===1?'':'s'} and ${moons} moon${moons===1?'':'s'}. To reach ${quota} of each, ${t} = ${targetValue===1?'sun ☀':'moon ☾'}.`]}
}
