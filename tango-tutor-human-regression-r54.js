...
function relationBalanceDetail(move,loc=locale()){
  ...
  if(loc==='fr'){const known=[];if(sunCells.length)known.push(`${sunCells.join('/')} = ${sunCells.length===1?'soleil':'soleils'}`);if(moonCells.length)known.push(`${moonCells.join('/')} = ${moonCells.length===1?'lune':'lunes'}`);return {where:`${a} ${symbol} ${b} ; équilibre ${unitText}.`,steps:[`${a} ${symbol} ${b} : symboles opposés, donc 1 soleil + 1 lune.`,`${known.join(' ; ')}. Avec ${a}/${b} : ${suns} soleil${suns>1?'s':''}, ${moons} lune${moons>1?'s':''}.`,`Pour obtenir ${quota} soleils / ${quota} lunes, ${t} doit être ${targetValue===1?'soleil ☀':'lune ☾'}.`]}}
  const known=[];if(sunCells.length)known.push(`${sunCells.join('/')} = ${sunCells.length===1?'sun':'suns'}`);if(moonCells.length)known.push(`${moonCells.join('/')} = ${moonCells.length===1?'moon':'moons'}`);return {where:`${a} ${symbol} ${b}; balance of ${unitText}.`,steps:[`${a} ${symbol} ${b}: opposite symbols, so 1 sun + 1 moon.`,`${known.join('; ')}. With ${a}/${b}: ${suns} sun${suns===1?'':'s'}, ${moons} moon${moons===1?'':'s'}.`,`To reach ${quota} suns / ${quota} moons, ${t} must be ${targetValue===1?'sun ☀':'moon ☾'}.`]}
}
