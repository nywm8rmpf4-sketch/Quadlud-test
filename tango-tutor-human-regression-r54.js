...
function relationBalanceDetail(move,loc=locale()){
  ...
  if(loc==='fr'){const known=[];if(sunCells.length)known.push(`${sunCells.join(', ')} = soleil${sunCells.length>1?'s':''}`);if(moonCells.length)known.push(`${moonCells.join(', ')} = lune${moonCells.length>1?'s':''}`);return {where:`Regarde ${a} ${symbol} ${b} dans ${unitText}.`,steps:[`${a} ${symbol} ${b} : ce couple contient un soleil et une lune.`,`${known.join(' ; ')}. Avec ce couple, ${unitText} compte ${suns} soleil${suns>1?'s':''} et ${moons} lune${moons>1?'s':''} ; pour atteindre ${quota}/${quota}, ${t} doit être ${targetValue===1?'soleil ☀':'lune ☾'}.`]}}
  const known=[];if(sunCells.length)known.push(`${sunCells.join(', ')} = sun${sunCells.length>1?'s':''}`);if(moonCells.length)known.push(`${moonCells.join(', ')} = moon${moonCells.length>1?'s':''}`);return {where:`Look at ${a} ${symbol} ${b} in ${unitText}.`,steps:[`${a} ${symbol} ${b}: this pair contains one sun and one moon.`,`${known.join('; ')}. With this pair, ${unitText} has ${suns} sun${suns===1?'':'s'} and ${moons} moon${moons===1?'':'s'}; to reach ${quota}/${quota}, ${t} must be ${targetValue===1?'sun ☀':'moon ☾'}.`]}
}
...
function renderRelationBalanceDetail(panel,move){const detail=relationBalanceDetail(move,locale());if(!detail||!panel)return false;const box=panel.querySelector?.('.walkthrough-explanation')||panel,ps=[...(box.querySelectorAll?.('p')||[])],where=ps[0],why=ps[1];if(!where||!why)return false;const loc=locale();where.innerHTML=`<b>${loc==='fr'?'Où regarder':'Where to look'} :</b> ${escapeHtml(detail.where)}`;why.innerHTML=`<b>${loc==='fr'?'Raisonnement :':'Reasoning:'}</b><br>${detail.steps.map(step=>`<span class="reason-step">${escapeHtml(step)}</span>`).join('')}`;return true}
