...
function relationBalanceDetail(move,loc=locale()){
  ...
  if(loc==='fr'){const known=[];if(sunCells.length)known.push(`${sunCells.join(' et ')} ${sunCells.length===1?'est soleil':'sont soleils'}`);if(moonCells.length)known.push(`${moonCells.join(' et ')} ${moonCells.length===1?'est lune':'sont lunes'}`);return {where:`${a} ${symbol} ${b} et ${unitText}.`,steps:[`${a} ${symbol} ${b} : 1 soleil, 1 lune. ${known.join(' ; ')} ; ${unitText} a donc ${suns} soleil${suns>1?'s':''} et ${moons} lune${moons>1?'s':''}. Pour atteindre ${quota} de chaque, ${t} = ${targetValue===1?'soleil ☀':'lune ☾'}.`]}}
  const known=[];if(sunCells.length)known.push(`${sunCells.join(' and ')} ${sunCells.length===1?'is a sun':'are suns'}`);if(moonCells.length)known.push(`${moonCells.join(' and ')} ${moonCells.length===1?'is a moon':'are moons'}`);return {where:`${a} ${symbol} ${b} and ${unitText}.`,steps:[`${a} ${symbol} ${b}: 1 sun, 1 moon. ${known.join('; ')}; ${unitText} therefore has ${suns} sun${suns===1?'':'s'} and ${moons} moon${moons===1?'':'s'}. To reach ${quota} of each, ${t} = ${targetValue===1?'sun ☀':'moon ☾'}.`]}
}
