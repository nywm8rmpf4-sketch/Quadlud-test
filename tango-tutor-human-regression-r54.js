...
function relationBalanceDetail(move,loc=locale()){
  ...
  if(loc==='fr'){const known=[];if(sunCells.length)known.push(`${sunCells.join(', ')} = soleil${sunCells.length>1?'s':''}`);if(moonCells.length)known.push(`${moonCells.join(', ')} = lune${moonCells.length>1?'s':''}`);return {where:`${unitText} : ${a} ${symbol} ${b}.`,steps:[`${a} ${symbol} ${b} = un soleil + une lune.`,`${known.join(' ; ')}. Total : ${suns} soleil${suns>1?'s':''}, ${moons} lune${moons>1?'s':''}. Pour équilibrer ${quota}/${quota}, ${t} doit être ${targetValue===1?'soleil ☀':'lune ☾'}.`]}}
  const known=[];if(sunCells.length)known.push(`${sunCells.join(', ')} = sun${sunCells.length>1?'s':''}`);if(moonCells.length)known.push(`${moonCells.join(', ')} = moon${moonCells.length>1?'s':''}`);return {where:`${unitText}: ${a} ${symbol} ${b}.`,steps:[`${a} ${symbol} ${b} = one sun + one moon.`,`${known.join('; ')}. Total: ${suns} sun${suns===1?'':'s'}, ${moons} moon${moons===1?'':'s'}. To balance ${quota}/${quota}, ${t} must be ${targetValue===1?'sun ☀':'moon ☾'}.`]}
}
