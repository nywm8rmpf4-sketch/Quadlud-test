...
'use strict';
const VERSION=5;
const HUMAN_PROOF_POLICY='tango-human-proof-minimal-v2';
...
function directProofCandidates(session,target,value){
  ...
  const seen=new Set(),out=[];for(const d of session.directDeductions()||[]){if(!d||!concludesMove(d,target,value))continue;const key=deductionKey(d);if(seen.has(key))continue;seen.add(key);out.push({deduction:copy(d),cost:humanProofCost(session,[d])})}
...
function selectDisplayProof(session,plan){
  const original=proofDeductions(plan).map(copy),baseCost=humanProofCost(session,original),base={schema:2,policy:HUMAN_PROOF_POLICY,kind:'engine-proof',target:Array.isArray(plan?.target)?plan.target.slice():null,value:plan?.value,deduction:copy(plan?.deduction||null),displayDeductions:original,replaced:false,witness:null,costVector:baseCost};
  ...
  // shortest sufficient human proof beat a longer causal chain.
  ...
  if(direct&&compareCostVector(direct.cost,baseCost)<0){
    return Object.freeze({schema:2,policy:HUMAN_PROOF_POLICY,kind:'simpler-direct-proof',target:plan.target.slice(),value:plan.value,deduction:direct.deduction,displayDeductions:Object.freeze([direct.deduction]),replaced:true,witness:null,replacedRule:String(plan.deduction.rule||''),costVector:direct.cost,replacedCostVector:baseCost})
  }
  ...
  if(compareCostVector(contradictionCost,baseCost)>=0)return Object.freeze(base);
  return Object.freeze({schema:2,policy:HUMAN_PROOF_POLICY,kind:'concrete-contradiction',target:plan.target.slice(),value:plan.value,deduction:alternative.deduction,displayDeductions:Object.freeze([alternative.deduction]),replaced:true,witness:alternative.witness,replacedRule:String(plan.deduction.rule||''),costVector:contradictionCost,replacedCostVector:baseCost})
}
...
function installCoachBridge(){
  ...
  const coherent=function(_rawDeduction){
    ...
    if(!isSame){current.hintFlow={kind:'tango-coherent-proof',boardKey,signature:sig,stage:1,flowVersion:6,plan:copy(plan),pedagogyView:projection.view,coachSections:projection.sections};coachUsage(1,presentation.technique);tangoFocusDeduction(d,false);showCoach(`<span class="coach-progress">1/2</span><b>${tr('where')} :</b> ${by.where?.text||presentation.explanation?.where||''}`);saveCurrent();return}
