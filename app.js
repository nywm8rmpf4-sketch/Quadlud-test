...
function togglePedagogicalSound(){let p=prefs();p.soundPedagogy=!p.soundPedagogy;savePrefs(p);return p.soundPedagogy}
function setSoundVolume(value){let p=prefs(),n=Math.max(0,Math.min(100,Number(value)||0));p.soundVolume=n/100;savePrefs(p);return Math.round(p.soundVolume*100)}
...
function settingsView(){
  ...
  <div class="setting-row"><span><b>${tr('soundPedagogy')}</b><small>${tr('soundPedagogySub')}</small></span><button class="btn" id="soundPedagogyToggle" aria-pressed="${p.soundPedagogy?'true':'false'}">${p.soundPedagogy?tr('on'):tr('off')}</button></div>
  <div class="setting-row"><span><b>${tr('soundVolume')}</b><small>${tr('soundVolumeSub')}</small></span><div class="audio-volume-control"><input id="soundVolume" type="range" min="0" max="100" step="5" value="${Math.round(p.soundVolume*100)}" aria-label="${tr('soundVolume')}" aria-valuetext="${Math.round(p.soundVolume*100)}%"><output id="soundVolumeValue" for="soundVolume">${Math.round(p.soundVolume*100)}%</output></div></div>
