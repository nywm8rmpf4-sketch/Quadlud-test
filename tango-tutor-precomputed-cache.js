...
(function(root,factory){
  ...
  const api=factory(
    root,
    isNode?require('./difficulty-rating.js'):root.DifficultyRating,
    isNode?require('./tango-played-move-planner.js'):root.QuadludTangoPlayedMovePlanner,
    isNode?require('./tango-tutor-cache-contract-r8.js'):root.QuadludTangoTutorCacheContractR8
  );
  ...
  const VERSION=3,DATA_SCHEMA=8,DATA_VERSION='tango-tutor-cache-r8-sync2',SOURCE_POOL_VERSION='tango-runtime-pool-r8';
  const DIFFICULTIES=Object.freeze(['easy','medium','hard','expert']);
  const BASE64URL='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_';
  let payload=null;const indices=new Map();
  const stats={hits:0,misses:0,dataRejects:0,rebuildRejects:0,contractRejects:0};
  ...
  function decodeSelection(data){return {selectionStatus:data?.[0]||null,candidateCount:Number(data?.[1])||0,humanCandidateCount:Number(data?.[2])||0,humanGlobalSelection:data?.[3]===1,frontierComplete:data?.[4]!==0,budgetHit:data?.[4]===0}}
  function validSelectionMeta(meta){return Array.isArray(meta)&&meta.length>=5&&Number.isFinite(Number(meta[1]))&&Number.isFinite(Number(meta[2]))&&(meta[3]===0||meta[3]===1)&&(meta[4]===0||meta[4]===1)}
  function validProofMeta(meta){return Array.isArray(meta)&&meta.length>=11&&typeof meta[0]==='string'&&(meta[1]===0||meta[1]===1)&&(meta[6]===0||meta[6]===1)&&Number.isFinite(Number(meta[7]))&&typeof meta[8]==='string'&&(meta[9]===0||meta[9]===1)&&(meta[10]===null||Number.isFinite(Number(meta[10])))}
  ...
  function contractMatches(data){
    return !!Contract&&Number(Contract.schema)===1&&Number(Contract.version)===1&&/^[0-9a-f]{64}$/.test(String(Contract.digest||''))&&
      String(data?.tutorContract?.digest||'')===String(Contract.digest)&&String(data?.tutorPlannerToken||'')===String(Contract.tutorPlannerToken||'')&&
      String(data?.humanPolicy||'')===String(Contract.humanPolicy||'')&&String(data?.proofPolicy||'')===String(Contract.proofPolicy||'')
  }
  function validateData(data){
    ...
    if(!contractMatches(data))return false;
    ...
    if(!Array.isArray(data.materializedDeductions)||!Array.isArray(data.selectionMeta)||!data.selectionMeta.length||data.selectionMeta.some(m=>!validSelectionMeta(m)))return false;
    if(!Array.isArray(data.proofMeta)||!data.proofMeta.length||data.proofMeta.some(m=>!validProofMeta(m)))return false;
  ...
  function resolveSeed(session,diff,kind,id,{display=false}={}){
    if(kind===1)return clone(payload?.materializedDeductions?.[id]||null);
    const signature=signatureFromId(id);if(!signature)return null;const matches=directMatches(session,diff,signature);if(matches.length!==1)return null;
    if(!display)return clone(matches[0]);const R=root?.QuadludTangoPlayedMoveRuntime;return typeof R?._test?.minimalDisplayDeduction==='function'?clone(R._test.minimalDisplayDeduction(matches[0])):clone(matches[0])
  }
  function decodeProof(meta,deduction,target,value){
    if(!validProofMeta(meta)||!deduction)return null;
    return {schema:3,policy:meta[8]||null,kind:meta[0]||'engine-proof',target:target.slice(),value,deduction:clone(deduction),displayDeductions:[clone(deduction)],replaced:meta[1]===1,witness:clone(meta[2]||null),replacedRule:meta[3]||'',costVector:Array.isArray(meta[4])?meta[4].slice():null,replacedCostVector:Array.isArray(meta[5])?meta[5].slice():null,traceCollapsed:meta[6]===1,discardedAlternativeCount:Number(meta[7])||0,humanRelationSupportCostCorrected:meta[9]===1,humanProofPreferenceTier:meta[10]===null?null:Number(meta[10])}
  }
  function tryPlan(session,diff){
    try{
      ...
      const P=planner(),tier=P.tierIndexForDifficulty(diff),plan=P._test.planFromFirstDeduction(session,tier,starting,{advancedStart:advanced,initialStateValidated:true});
      ...
      const displayDeduction=resolveSeed(session,diff,step[7],step[8],{display:true}),displayProof=decodeProof(payload.proofMeta[step[9]],displayDeduction,target,value);
      if(!displayDeduction||!displayProof){stats.rebuildRejects++;stats.misses++;return null}
      stats.hits++;return {...plan,...decodeSelection(payload.selectionMeta[step[6]]),displayDeduction,displayProof,precomputedTutorCache:true,precomputedTutorFingerprint:fingerprint,precomputedTutorAdvanced:advanced,precomputedTutorContractDigest:String(payload.tutorContract.digest)}
