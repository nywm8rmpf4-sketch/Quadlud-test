...
function premiseCells(proof){
  const out=[];for(const p of proof?.premises||[]){if(Array.isArray(p?.cell))out.push(p.cell);if(Array.isArray(p?.a))out.push(p.a);if(Array.isArray(p?.b))out.push(p.b)}return uniqueCells(out)
}
function projectionForMove(move){
  const proof=move?.causalProof,step=causalStep(move);if(!proof||!step)return null;
  const roles=step.cellRoles||{},hypotheticalMoves=[];
  if(step.hypothetical&&step.kind==='deduction'){
    for(const cell of step.producedCells||[])hypotheticalMoves.push({cell:copy(cell),sequenceIndex:Number.isInteger(step.sequenceIndex)?step.sequenceIndex:null})
  }
  return Object.freeze({
    proofSchema:proof.schema||null,stepId:step.id,kind:step.kind,hypothetical:step.hypothetical===true,
    contextCells:premiseCells(proof),focusCells:uniqueCells(roles.focusCells||step.involvedCells||[]),
    hypothesisCells:uniqueCells(roles.hypothesisCells||[]),contradictionCells:uniqueCells(roles.contradictionCells||[]),
    conclusionCells:uniqueCells(roles.conclusionCells||[]),hypotheticalMoves:Object.freeze(hypotheticalMoves),
    sequenceIndex:Number.isInteger(step.sequenceIndex)?step.sequenceIndex:null
  })
}
...
function currentEntry(){
  const group=currentGroup(),entries=Array.isArray(group?.entries)?group.entries:[],index=Number(session()?.navigation?.proofStepIndex)||0;
...
function clear(boardNode){
  ...
  boardNode.removeAttribute?.('data-causal-proof-kind');boardNode.removeAttribute?.('data-causal-proof-step')
}
...
function decorate(){
  ...
  const move=entryMove(currentEntry()),projection=projectionForMove(move);if(!projection)return false;
  applyCells(boardNode,projection.contextCells,'walkthrough-causal-context');
  applyCells(boardNode,projection.focusCells,'walkthrough-causal-focus');
  applyCells(boardNode,projection.hypothesisCells,'walkthrough-causal-hypothesis');
...
function install(){
  if(installed)return true;if(typeof renderWalkthrough!=='function')return false;
  previousRender=renderWalkthrough;const wrapped=function(options={}){const result=previousRender(options);decorate();return result};wrapped.__quadludCausalProofProjection=true;renderWalkthrough=wrapped;
